...
public class Maratonac {
	...
	public static double prosjecnoVrijeme() {
		double suma = 0;
		double prosjek = 0;
		for(int i = 0; i < ucesnici.size(); i++) {
			suma += ucesnici.get(i).vrijemeUcesnika;
		}
		prosjek = suma / ucesnici.size();
		return prosjek;
		
	}
	
	public static void manjiOd300() {
		for(int i = 0; i < ucesnici.size(); i++) {
			if(ucesnici.get(i).vrijemeUcesnika < 300) {
				System.out.println(ucesnici.get(i).imeUcesnika + " " +  ucesnici.get(i).vrijemeUcesnika);
			}
		}
	}
